#!/usr/bin/env bash
# stage 4: post-routing vs final equivalence check
...
WORK_DIR="$SCRIPT_DIR/work_stage4"
LOG_DIR="$SCRIPT_DIR/logs"
LOG_FILE="$LOG_DIR/stage4.log"

SRAM_BB="$PROJECT_DIR/src/sky130_sram_blackbox.sv"

GOLD_NL="$RUN_DIR/45-openroad-detailedrouting/l1_cache_core.nl.v"
GATE_NL="$RUN_DIR/final/nl/l1_cache_core.nl.v"

echo "=== LEC Stage 4: Post-Routing vs Final ==="
echo "Gold: $GOLD_NL"
echo "Gate: $GATE_NL"
...
# stage 4: post-routing vs final, equiv_simple -seq 10
...
if yosys -Q -T -l "$LOG_FILE" -s "$WORK_DIR/run_equiv.ys" 2>&1; then
    echo "stage 4 PASS: routing matches final"
    grep -A5 "equiv_status" "$LOG_FILE" | tail -6
    exit 0
else
    echo "stage 4 FAIL: routing does not match final"
